...
conn = sqlite3.connect('freelancer.sqlite')
cur = conn.cursor()
...
for i in range(0, number_pages):
    ...
    for job in job_cards:
        ...
        print(info)
        job_list.append((title, days, checkNone(pp_hr), checkNone(pp_range), checkNone_2(pp_range), checkNone(pp_normal), bid, rating, country))
        count += 1
        cur.executemany('''INSERT OR IGNORE INTO Jobs (Title, Days, Price_hr, Price_range_Min, Price_range_Max, Price, Bid, Rating, Country)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', job_list)
        conn.commit()
